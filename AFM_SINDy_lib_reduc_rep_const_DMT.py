...
def run_sindy_model(args):
    try:
        ...
        constraint_rhs_rep, constraint_lhs_rep, dt, traject, counter, lock, number_lambda_nu_comb, script_dir) = args
        ...
        phd.fit_constrained_SINDy_model(
            ...
            constraint_rhs_array=constraint_rhs_rep,
            constraint_lhs_array=constraint_lhs_rep,
            ...
            save_folder_name=f'repulsive_lib_reduc_DMT', #/cluster_{cluster_num}' #for additional folder per cluster
            model_filename=f"constr_repmodel_DMT_SN_{serial_number}",
            save_model=True  
        )
...
if __name__ == "__main__":
    ...
    d1 = 0.0034 / 2
    d2 = 4.057 / 2
    forcing_freq = 1.002 #* 0.07 # 0.175, 0.3 or 1.2  #equivalent to \bar{Omega}
    y_bar = 0.05585 
    F_act = B1*(forcing_freq**2)*y_bar 

    dt = 0.01 
    t_train_DMT = np.arange(0, 1000, dt) #before until 1000
    ...
    AFM_DMT_ivp = solve_ivp(
        ...
        init_cond_DMT,
        t_eval=t_train_DMT,
        args=(a0, d1, d2, C1, C2, B1, y_bar, forcing_freq),
        rtol=1e-6,
        atol=1e-8,
        method='RK45',
        dense_output=True
    ...
    x_train_DMT_mult_traj, init_cond_list = phd.generate_different_trajectories(
        dynamical_system=AFM_w_DMT,
        t_train=t_train_DMT,
        DOF=3,
        num_trajectories=3,
        noisy_trajectories=False,
        parameters=(a0, d1, d2, C1, C2, B1, y_bar, forcing_freq),
        noise_level=None,
    ...
    ### -------------------- 4. SINDy Regime Analysis assuming Repulsive Regime --------------------
    ...
    SINDy_constraints_df.loc[2] = [1, 'e2', -d2]
    ...
    constraint_rhs_rep = SINDy_constraints_df["Constraint Value"].to_numpy()
    constraint_lhs_rep = np.zeros((len(constraint_rhs_rep), n_targets * n_features)) # One row per constraint, one column per coefficient
    ...
    for index_number in SINDy_constraints_df.index.to_list():
        ...
        constraint_lhs_rep[index_number, cand_funct_index + pos_in_matrix ]= 1 
    ...
    for cluster_num in cluster_nums:
        for i, combination in enumerate(lambda_nu_combinations):
            args.append((i, combination, cluster_num, mult_traj_clusters_sections, lib_concat, feature_names,
                        constraint_rhs_rep, constraint_lhs_rep, dt, traject, counter, lock, number_lambda_nu_comb, script_dir))
