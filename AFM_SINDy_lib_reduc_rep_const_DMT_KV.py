...
def run_sindy_model(args):
    try:
        ...
        phd.fit_constrained_SINDy_model(
            ...
            save_folder_name=f'repulsive_lib_reduc_viscDMT', #/cluster_{cluster_num}' #for additional folder per cluster
            model_filename=f"constr_repmodel_DMTvisc_SN_{serial_number}",
            save_model=True  
        )
