...
def run_sindy_model(args):
    try:
        ...
        phd.fit_constrained_SINDy_model(
            ...
            save_folder_name=f'attractive_lib_reduc_DMT', #/cluster_{cluster_num}' #for additional folder per cluster
            model_filename=f"constr_attmodel_DMT_SN_{serial_number}",
            save_model=True  
        )
